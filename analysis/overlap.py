...
from fact_cluster import get_factor_df
...
if __name__ == '__main__':
    ...
    mycol = [x for x in c.columns if ('text' in x ) or ('fact' in x) ][0]
    ...
    for group in c[mycol].unique():
        ...
        print gdf.name[0:10]
        #print names
        names = ",".join( gdf.name[0:10].values.tolist())
        #names=0
    ...
    sns.heatmap(P, annot=False ,annot_kws={"size": 12})
    plt.xlabel("gics")
    plt.savefig('p.'+fi+'.pdf')
    plt.clf()
    #sns.heatmap(B, annot=True  ,annot_kws={"size": 12})
    sns.heatmap(B, annot=False  ,annot_kws={"size": 12})
    plt.xlabel("gics")
